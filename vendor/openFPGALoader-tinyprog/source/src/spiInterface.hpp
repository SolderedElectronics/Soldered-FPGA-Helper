...
class SPIInterface {
 public:
	...
	/*!
	 * \brief send a command with distinct write/read lengths.
	 * \param[in] cmd: command/opcode to send
	 * \param[in] tx: buffer to send
	 * \param[in] tx_len: number of byte to send (cmd not comprise)
	 * \param[out] rx: buffer for read access
	 * \param[in] rx_len: number of byte to read
	 * \return 0 when success
	 */
	virtual int spi_put(uint8_t cmd, const uint8_t *tx, const uint32_t tx_len, uint8_t *rx,
		const uint32_t rx_len)
	{
		const uint32_t tx2_len = (tx) ? tx_len : 0;
		const uint32_t rx2_len = (rx) ? rx_len : 0;
		const uint32_t len = tx2_len + rx2_len;

		std::vector<uint8_t> tx2(len, 0);
		std::vector<uint8_t> rx2(len);

		if (tx)
			memcpy(tx2.data(), tx, tx2_len);

		const int ret = spi_put(cmd, (tx) ? tx2.data() : NULL,
			(rx) ? rx2.data() : NULL, len);

		if (ret == 0 && rx)
			memcpy(rx, rx2.data() + tx2_len, rx2_len);
		return ret;
	}

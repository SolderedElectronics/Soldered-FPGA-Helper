...
#ifdef ENABLE_TINYPROG
int tinyprog_comm(const struct arguments &args, const cable_t &cable,
	target_board_t *board);
#endif
...
int main(int argc, char **argv)
{
	...
	struct arguments args = {
			...
			/* TinyProg cable */
			false, false, false // tinyprog, boot, bootloader_update
	};
	/* parse arguments */
	...
	if (args.tinyprog || (board && board->mode == COMM_TINYPROG)) {
#ifdef ENABLE_TINYPROG
		return tinyprog_comm(args, cable, board);
#else
		printError("TinyProg: disabled at build time");
		return EXIT_FAILURE;
#endif
	}
...
int tinyprog_comm(const struct arguments &args, const cable_t &cable,
	target_board_t *board)
{
	bool do_boot = args.boot;
	try {
		TinyProg tp(args.device, cable, args.freq, args.detect, args.verbose);
		if (args.detect)  // Nothing to do: detect is performed in ctor
			return EXIT_SUCCESS;

		if (args.prg_type == Device::RD_FLASH) {
			tp.set_out_file(args.bit_file);
			if (!tp.dump(args.offset, args.file_size))
				return EXIT_FAILURE;
			return EXIT_SUCCESS;
		}

		if (args.bootloader_update) {
			if (tp.bootloader_update(args.user_flash, args.bit_file))
				return EXIT_SUCCESS;
			else
				return EXIT_FAILURE;
		}
		if (!args.bit_file.empty()) {
			do_boot = true;
			if (!tp.program(args.bit_file, "", 0, 0, false, args.verify))
				return EXIT_FAILURE;
		}

		if (!args.user_flash.empty()) {
			do_boot = true;
			if (!tp.program("", args.user_flash, 0, 0, false, args.verify))
				return EXIT_FAILURE;
		}
	...
	jtag_pins_conf_t *pins_config)
{
	...
	try {
		...
		options
			...
			("user-flash", "User flash file (Gowin LittleBee FPGA and TinyProg Cable only)",
				cxxopts::value<std::string>(args->user_flash))
			("tinyprog", "TinyProg Cable mode",
				cxxopts::value<bool>(args->tinyprog))
			("bootloader-update", "TinyProg Cable bootloader Update",
				cxxopts::value<bool>(args->bootloader_update))

...
class SPIFlash {
	public:
		...
		/*!
		 * \brief return current JEDEC ID value
		 * \return JEDEC ID read by read_id()
		 */
		uint32_t get_jedec_id() const noexcept { return _jedec_id; }

		/*!
		 * \brief read one Security Register
		 * \param[in] id: security register index
		 * \param[out] buffer: destination buffer
		 * \param[in] len: number of byte to read (max 256)
		 * \return false on invalid parameter, unknown index or SPI transfer
		 *         failure, true otherwise
		 */
		bool read_security_register(uint8_t id, uint8_t *buffer,
				uint32_t len = 256);

...
int SPIFlash::sectors_erase(int base_addr, int size)
{
	...
	for (int addr = start_addr; addr < end_addr; addr += step) {
		...
		/* Use 4KB sector erase when:
		 * - no 64KB block erase is available, or
		 * - addr is not yet 64KB-aligned (head: align up to next 64KB boundary), or
		 * - less than 64KB remains before end_addr (tail: finish with 4KB sectors).
		 * Use 64KB block erase when addr is 64KB-aligned and enough room remains. */
		if (!sector_rdy ||
			(subsector_rdy && ((addr & 0xffff) != 0 || addr + 0x10000 > end_addr))) {
			step = 0x1000;
...
int SPIFlash::read(int base_addr, uint8_t *data, int len)
{
	uint8_t tx[4];
	uint32_t i = 0;
	const uint8_t read_cmd = (base_addr <= 0xffffff) ? FLASH_READ : FLASH_4READ;
	...
	int ret = _spi->spi_put(read_cmd, tx, i, (data) ? data : NULL, len);
	if (ret != 0)
		printf("error\n");
	return ret;
}
...
bool SPIFlash::read_security_register(uint8_t id, uint8_t *buffer, uint32_t len)
{
	/* A flash entry is required to access id<->addr and
	 * opcode to read these registers
	 */
	if (!_flash_model) {
		printError("Read Security Register: Unknown flash model");
		printError("    Please add an entry in spiFlashdb.hpp");
		return false;
	}
	...
	if (_flash_model->sec_reg_addr.empty()) {
		printError("Read Security Register: sec_reg_addr is empty in flash model");
		printError("    Please update flash entry in spiFlashdb.hpp");
		return false;
	}
	const std::map<uint8_t, uint32_t>::const_iterator it = _flash_model->sec_reg_addr.find(id);
	if (it == _flash_model->sec_reg_addr.end()) {
		printError("Read Security Register: flash has no security register with id " + std::to_string(id));
		return false;
	}
	const uint32_t addr = it->second;
	const uint8_t read_cmd = _flash_model->sec_reg_read_cmd;
	if (read_cmd == 0) {
		printError("Read Security Register: sec_reg_read_cmd is not defined");
		printError("    Please update flash entry in spiFlashdb.hpp");
		return false;
	}

	/* 3-byte addr + 1 dummy byte */
	uint8_t tx[4] = {
	...
	if (_spi->spi_put(read_cmd, tx, 4, buffer, len) != 0)
		return false;

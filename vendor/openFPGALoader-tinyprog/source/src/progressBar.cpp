...
#ifdef _WIN32
/* On Windows, isatty() often returns 0 for a real console.
 * GetConsoleMode() is the reliable way to detect interactive output. */
...
#else
static bool stdout_is_tty() { return isatty(STDOUT_FILENO) == 1; }
#endif
...
ProgressBar::ProgressBar(const std::string &mess, int maxValue,
		...
		_is_tty(_force_terminal_mode || stdout_is_tty())
{
}
